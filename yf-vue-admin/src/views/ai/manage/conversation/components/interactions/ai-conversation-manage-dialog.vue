...
import {AiConversationForm} from "@/api/ai/conversation/type";
...
// 组件定义
defineOptions({
  name: "AiConversationManageDialog",
  inheritAttrs: false,
});
// 组件 props & emits
...
const {
  saveData,
  updateData
} = useCrudActions<string, AiConversationForm>(undefined, AiConversationAPI.UPDATE.request, undefined, undefined);
// 初始校验规则
const initialForm: AiConversationForm = {
  title: undefined,
}
const rules: FormRules = {}
// 数据
const formRef = ref<FormInstance | null>(null);          // 字典数据表单
const form = ref<AiConversationForm>({...initialForm});
