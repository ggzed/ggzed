<template>
  <el-card class="table-container">
    ...
    <template #header>
      <div>
        <el-button v-permission="[AiMessageAPI.SAVE.permission]"
                   :circle="device === DeviceEnum.MOBILE"
                   plain
                   type="success"
                   @click="openAiMessageDialog()">
          <el-icon>
            <plus/>
          </el-icon>
          <span v-show="device !== DeviceEnum.MOBILE"> 新增 </span>
        </el-button>
        <el-button v-permission="[AiMessageAPI.UPDATE.permission]"
                   :circle="device === DeviceEnum.MOBILE"
                   :disabled="ids.length !== 1"
                   plain
                   type="warning"
                   @click="openAiMessageDialog(ids[0])">
          <el-icon>
            <edit/>
          </el-icon>
          <span v-show="device !== DeviceEnum.MOBILE"> 修改 </span>
        </el-button>
        <el-button v-permission="[AiMessageAPI.DELETE.permission]"
                   :circle="device === DeviceEnum.MOBILE"
    ...
    >
      ...
      <el-table-column align="center" label="角色" prop="role">
        <template #default="scope">
          <el-tag type="primary">
            {{ props.dictData['ai_role'][scope.row?.role] }}
          </el-tag>
        </template>
      </el-table-column>
      <el-table-column align="center" label="聊天内容" prop="content"/>
      <el-table-column align="center" label="发送时间" prop="sendTime"/>
      <el-table-column v-permission="[AiMessageAPI.UPDATE.permission, AiMessageAPI.DELETE.permission]"
                       :fixed="device === DeviceEnum.MOBILE ? false : 'right'" align="center" label="操作"
                       width="180">
        <template #default="scope">
          <el-button
              v-permission="[AiMessageAPI.UPDATE.permission]"
              link
              size="small"
              type="warning"
              @click="openAiMessageDialog(scope.row.id)"
          >
            <el-icon>
              <edit/>
            </el-icon>
          ...
          >
          <el-button
              v-permission="[AiMessageAPI.DELETE.permission]"
              link
              size="small"
              type="danger"
...
import {AiMessageForm, AiMessagePageQuery, AiMessagePageVO} from "@/api/ai/message/type";
...
const {
  deleteData
} = useCrudActions<string, AiMessageForm>(undefined, undefined, AiMessageAPI.DELETE.request, undefined);
// 数据
...
/**
 * 打开Ai 消息记录表数据模态框
 * @param id AiMessage主键
 */
function openAiMessageDialog(id?: string) {
  currentClickRowId.value = id;
  if (id) {
    openDialog("修改Ai 消息记录表数据");
  } else {
    openDialog("新增Ai 消息记录表数据");
  }
}
</script>
